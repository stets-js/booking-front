...
const getTeamCalendarWeek = createAsyncThunk(
  GET_WEEK,
  ({ weekId, team, manager, course, group }, { rejectWithValue }) => {
    return getTeamsWorkWeek(weekId, team, manager, course, group)
      .then((data) => data)
      .catch((data) => {
        error(
          `${
...
const getCallerWeekByCourse = createAsyncThunk(
  GET_WEEK,
  ({ weekId, courseId, manager }, { rejectWithValue }) => {
    try {
      return getCallerWorkWeekByCourse(weekId, courseId, manager);
    } catch (error) {
