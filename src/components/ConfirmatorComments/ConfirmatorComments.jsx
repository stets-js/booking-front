...
import { updateAppointmentComment } from "../../redux/confirmator/confirmator-reducers";
import { addCallToAppointment, putNewComment } from "../../helpers/appointment/appointment";
...
const ConfirmatorComments = ({ value, dispatch }) => {
  ...
  const handleCommentChange = (item) => {
    ...
    setModalComment(item.appointment_id);
    setCommentText(item.comments || "");
  };

  const handleSaveComment = async (item) => {
    dispatch(updateAppointmentComment(item.appointment_id, commentText));
    const data = new FormData();
      data.append("crm_link", item.crm_link);
      data.append("appointment_id", item.appointment_id);
    ...
    try {
      await putNewComment(data).finally(() => {
        setCommentText("");
      }).catch((err) => {
      ...
      error("Error saving comment", err);
      setModalComment(null); 
    }
        ...
        <div key={item.appointment_id} className={styles.comment__wrapper}>
            ...
            value[item.appointment_id] !== "canceled" && (
              <>
                <div
                  ...
                  onClick={() => handleCommentChange(item)}
                >
                  <svg
                  ...
                  >
                    ...
                    <path
                      fill="#fff"
                      d="M15 7.75A1.25 1.25 0 1 0 15 10.25 1.25 1.25 0 1 0 15 7.75zM16 21L16 12 13 12 13 13 14 13 14 21 13 21 13 22 17 22 17 21z"
                    ></path>
                  </svg>
                  {hoveredItem === item.appointment_id && (
                    <div className={styles.commentsWindow}>{item.comments}</div>
                  )}{" "}
                  <p>
                  ...
                  {modalComment === item.appointment_id && (
                    <div className={styles.modal_comment} ref={modalRef}>
                      <textarea
                        type="text"
                        value={commentText}
                        onChange={(e) => setCommentText(e.target.value)}
                      />
                      <button onClick={()=>handleSaveComment(item)}>Save</button>
                    </div>
                  )}
                </div>
                <div
          ...
          {value[item.appointment_id] === "confirmed" && (
            <input
              ...
              onBlur={() =>
                confirm &&
                setConfirmation(item.slot_id, 4, confirm)
                  .then(() => success("Successfully confirmed"))
          ...
          {value[item.appointment_id] === "canceled" && (
            <div className={styles.comment__reject_btn}>
              ...
              >
                ...
                <option value="no parents attending">
                ...
                <option value="child has no interest">
